...
pub fn GroveAdminTab(
    ...
    {
        let enable_invites_action = enable_invites_action.clone();
        let grove = grove_resource.clone();
    ...
    {
        let disable_invites_action = disable_invites_action.clone();
        let grove = grove_resource.clone();
    ...
    {
        let pandas_resource = pandas_resource.clone();
            ...
            if let Some(Ok(pandas_in_grove)) = pandas_resource.get() {
                pandas.set(
                    pandas_in_grove
                        .iter()
                        .map(|panda| {
                            return (panda.id.to_string(), panda.display_name.clone());
                        })
                        .collect::<Vec<_>>(),
                );
                selected_mods.set(
    ...
    let on_delete = {
        let delete_grove = delete_grove_action.clone();
            ...
            if let Some(Ok(grove)) = grove_resource.get() {
                confirm(
                    ...
                    Some(Callback::new(move |_| {
                        delete_grove.dispatch(DeleteGroveAction { id: grove_id.get() })
                    })),
                    None,
                )
            }
    ...
    let on_unban = {
        Callback::new(move |user: GroveUser| {
            confirm(
    ...
    {
        let delete_grove_action = delete_grove_action.clone();
    ...
    {
        let unban_panda_action = unban_panda_action.clone();
        let banned_pandas_resource = banned_pandas_resource.clone();
            ...
            <div class="pandas-grove__management">
                ...
                <Show
                    when={
                        ...
                        move || if let Some(Ok(grove)) = grove_resource.get() {
                ...
                >
                    <p>
                        ...
                        <br />
                        <a href={grove_resource.clone().get().unwrap().unwrap().get_invite_link().unwrap()}>{ format!("https://bambushain.app{}", grove_resource.clone().get().unwrap().unwrap().get_invite_link().unwrap()) }</a>
                    </p>
                    <p>
                        ...
                        <br />
                        <Button label="Einladungen deaktivieren" on:click={disable_invite} />
                    </p>
                </Show>
                <h2>{"Modverwaltung"}</h2>
                <p>
                ...
                <ActionForm
                    buttons={Box::new(|| view!{
                        <Button is_submit={true} label="Mods speichern" />
                    }.into())}
                ...
                <Show when={
                    ...
                    move || {
                        banned_pandas_resource
                            .get()
                            .is_some_and(|panned_pandas| panned_pandas
                                .is_ok_and(|banned_pandas| !banned_pandas.is_empty()))
                    }
                }>
                    ...
                    <Table
                        headers={vec!["Name".to_string(), "Email".to_string(), "Discord".to_string(), "Aktionen".to_string()]}
                    >
                    {move ||    banned_pandas_resource.get().unwrap().unwrap().into_iter().filter_map(|user| Some(view! {
                            <tr>
                                <td>{user.display_name.clone()}</td>
                                <td>{user.email.clone()}</td>
                                <td>{user.discord_name.clone()}</td>
                                <td>
                                    <Button label="Ban aufheben" on:click={move |_| on_unban.call(user.clone())} />
                                </td>
                            </tr>
                        })).collect::<Vec<_>>()}
                    </Table>
